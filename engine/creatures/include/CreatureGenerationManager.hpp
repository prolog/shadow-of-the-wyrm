...
class CreatureGenerationManager
{
  public:
    ...
    CreatureGenerationMap generate_creature_generation_map(const TileType map_terrain_type, const int danger_level, const Rarity rarity);
    std::shared_ptr<Creature> generate_creature(ActionManager& am, CreatureGenerationMap& creature_generation_map);
    
  protected:
    bool does_creature_match_generation_criteria(const CreatureGenerationValues& cgv, const TileType map_terrain_type, const int danger_level, const Rarity rarity);
};
