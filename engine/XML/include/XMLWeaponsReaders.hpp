...
class XMLWeaponsReader : public XMLWearableReader
{
  ...
  protected:
    ...
    void parse(WeaponPtr weapon, GenerationValues& gv, const XMLNode& ranged_weapon_node, const bool force_ascii);
};
...
class XMLAmmunitionReader : public XMLWeaponsReader
{
  ...
  protected:
    friend class XMLItemsReader;
    void parse(WeaponPtr ammunition, GenerationValues& gv, const XMLNode& ammunition_node, const bool force_ascii);
};
