...
Spell XMLSpellsReader::parse(const XMLNode& spell_node)
{
  ...
  if (!spell_node.is_null())
  {
    ...
    string spell_description_sid = XMLUtils::get_child_node_value(spell_node, "DescriptionSID");
    ...
    uint radius = static_cast<uint>(XMLUtils::get_child_node_int_value(spell_node, "Radius", SpellConstants::DEFAULT_BEAM_RADIUS));
    SpellShape spell_shape = SpellShapeFactory::create_spell_shape(shape_type, radius);
    ...
    spell.set_spell_description_sid(spell_description_sid);
