...
class DecisionStrategy : public ISerializable
{
  public:
    ...
    virtual CommandPtr get_decision(const bool reprompt_on_cmd_not_found, const std::string& this_creature_id, CommandFactoryPtr command_factory, KeyboardCommandMapPtr keyboard_commands, std::shared_ptr<Map> view_map = std::shared_ptr<Map>() /* optional - only used when getting a decision on the main map, and only for non-player characters. */, int* key_p = 0) = 0;
...
typedef std::shared_ptr<DecisionStrategy> DecisionStrategyPtr;
