...
class Search
{
  ...
  protected:
    ...
    SearchNode remove_front(std::list<SearchNode>& search_queue);
    
    // Create a list of search nodes given a coordinate
    std::list<SearchNode> make_search_nodes(MapPtr view_map, std::set<Coordinate>& visited, const Coordinate& c, const SearchNode* const parent = NULL);
    
    // Create a list of search nodes given a search node - this will fully populate
    // depth and path cost details.
    std::list<SearchNode> make_search_nodes(MapPtr view_map, std::set<Coordinate>& visited, const SearchNode& sn);
