...
class Search
{
  ...
  protected:
    ...
    std::list<SearchNode> make_search_nodes(MapPtr view_map, const Coordinate& c);
    
    // Create a list of search nodes given a search node - this will fully populate
    // depth and path cost details.
    std::list<SearchNode> make_search_nodes(MapPtr view_map, const SearchNode& sn);
    
    // Variations on this give different search types (BFS, DFS, etc).
    virtual std::list<SearchNode> queueing_fn(std::vector<SearchNode>& search_nodes) = 0;
};
