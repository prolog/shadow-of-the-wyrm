...
TEST(SW_Engine_Calculator_ParalysisCalculator, calc_pct_chance_paralysis)
{
  CreaturePtr creature = std::make_shared<Creature>();
  creature->set_health(5);
  creature->get_resistances().set_resistance_value(DamageType::DAMAGE_TYPE_LIGHTNING, 1.0);
  ...
  creature->get_resistances().set_resistance_value(DamageType::DAMAGE_TYPE_LIGHTNING, 0.5);
