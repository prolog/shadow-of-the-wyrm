...
TEST(SL_Engine_Calculators_APRegenerationCalculator, minutes_per_ap)
{
  ...
  EXPECT_EQ(static_cast<uint>(40 * bed->get_ap_regeneration_multiplier()), aprc.calculate_minutes_per_ap_tick(creature, tile));

  will.set_base(20);
  will.set_current(20);
  creature->set_willpower(will);

  EXPECT_EQ(static_cast<uint>(40 * bed->get_ap_regeneration_multiplier() * 0.80), aprc.calculate_minutes_per_ap_tick(creature, tile));
