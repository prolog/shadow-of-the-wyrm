...
TEST(SL_Engine_Calculators_HPRegenerationCalculator, minutes_per_hp)
{
  ...
  EXPECT_EQ(static_cast<uint>(30 * bed->get_hp_regeneration_multiplier()), hprc.calculate_minutes_per_hp_tick(creature, tile));

  health.set_base(25);
  health.set_current(25);
  creature->set_health(health);

  EXPECT_EQ(static_cast<uint>(30 * bed->get_hp_regeneration_multiplier() * 0.75), hprc.calculate_minutes_per_hp_tick(creature, tile));
