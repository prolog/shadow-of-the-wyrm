...
TEST(SL_Engine_Calcalators_ResistancesCalculator, calculate_equipment_resistances)
{
  ...
  i1resist.set_resistance_value(DAMAGE_TYPE_LIGHTNING, -0.35);
  i1resist.set_resistance_value(DAMAGE_TYPE_ARCANE, 0.04);
  item->set_resistances(i1resist);

  Resistances i2resist;
  i2resist.set_all_resistances_to(0);
  i2resist.set_resistance_value(DAMAGE_TYPE_SLASH, 0.20);
  i2resist.set_resistance_value(DAMAGE_TYPE_LIGHTNING, 0.10);
  i2resist.set_resistance_value(DAMAGE_TYPE_ARCANE, 0.14);
  item2->set_resistances(i2resist);

  Equipment& eq = creature->get_equipment();
  eq.set_item(item2, EQUIPMENT_WORN_HEAD);
  eq.set_item(item, EQUIPMENT_WORN_RIGHT_FINGER);
  ...
  EXPECT_DOUBLE_EQ(1.25, calc_resist.get_resistance_value(DAMAGE_TYPE_LIGHTNING));
  EXPECT_DOUBLE_EQ(0.80, calc_resist.get_resistance_value(DAMAGE_TYPE_SLASH));
  EXPECT_DOUBLE_EQ(0.82, calc_resist.get_resistance_value(DAMAGE_TYPE_ARCANE));
}