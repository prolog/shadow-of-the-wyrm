...
TEST(SW_Engine_Calculators_LineOfSightCalculator, tod_penalty)
{
  ...
  int los_length = losc.calculate_los_length(creature, TimeOfDayType::TIME_OF_DAY_DAY);
  EXPECT_EQ(6, los_length);

  los_length = losc.calculate_los_length(creature, TimeOfDayType::TIME_OF_DAY_DUSK);
  EXPECT_EQ(5, los_length);

  los_length = losc.calculate_los_length(creature, TimeOfDayType::TIME_OF_DAY_DAWN);
  EXPECT_EQ(5, los_length);

  los_length = losc.calculate_los_length(creature, TimeOfDayType::TIME_OF_DAY_NIGHT);
