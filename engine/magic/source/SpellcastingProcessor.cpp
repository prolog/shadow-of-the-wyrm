...
bool SpellcastingProcessor::process(SpellShapeProcessor* spell_processor, CreaturePtr caster, MapPtr current_map, const Coordinate& caster_coord, const Direction spell_direction, const Spell& spell, const int bonus, const ItemStatus effect_status, const AttackType attack_type) const
{
  ...
  return spell_processor->process_damage_and_effect(caster, affected_coord_tiles, spell, bonus, effect_status, &game.get_action_manager_ref(), attack_type);
}
