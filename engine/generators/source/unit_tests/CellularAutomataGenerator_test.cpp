...
{
  public:
    vector<pair<int, int> > get_adjacent_map_coordinates(const Dimensions& dim, const int row, const int col) { return MapUtils::get_adjacent_map_coordinates(dim, row, col); }
};
...
TEST(SL_Engine_Generators_CellularAutomataGenerator, adjacent_map_coordinates_size)
{
  ...
  int row  = 0;
  int col  = 0;
  vector<pair<int, int> > adjacent_coords = MapUtils::get_adjacent_map_coordinates(d, row, col);
  ...
  row = 1;
  col = 0;
  adjacent_coords = MapUtils::get_adjacent_map_coordinates(d, row, col);
