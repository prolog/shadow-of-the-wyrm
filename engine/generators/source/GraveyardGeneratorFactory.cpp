...
{
  GeneratorPtr ordered_gen = std::make_unique<OrderedGraveyardGenerator>(map_exit_id);
  return ordered_gen;
}
...
{
  GeneratorPtr scattered_gen = std::make_unique<ScatteredGraveyardGenerator>(map_exit_id);
