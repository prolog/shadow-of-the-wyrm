...
void VaultPopulator::populate_vault_creatures(MapPtr map, const TileType tile_type, const vector<Coordinate>& coords, const int danger_level, const Rarity rarity)
{
  ...
  CreatureGenerationList generation_list = cgm.generate_creature_generation_map({ tile_type }, map->get_permanent(), map->is_islet(), min_danger_level, danger_level, rarity, map->get_properties()).get();
