...
bool ShopSectorFeature::generate_feature(MapPtr map, const Coordinate& start_coord, const Coordinate& end_coord)
{
  ...
  if (map != nullptr)
  {
    ...
    BuildingGenerationParameters bgp(start_coord.first, end_coord.first, start_coord.second, end_coord.second, door_dir, false, bcf.create_shop_features(), bcf.create_shop_item_ids());
    ShopGenerator sg;
