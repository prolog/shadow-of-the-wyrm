...
GeneratorPtr SettlementGeneratorFactory::create_settlement_generator(const SettlementType settlement_type, MapPtr base_map)
{
  ...
  switch(settlement_type)
  {
    case SETTLEMENT_TYPE_ORDERLY_VILLAGE:
      settlement_generator = boost::make_shared<SettlementGenerator>(base_map);
      break;
    case SETTLEMENT_TYPE_HAMLET:
      settlement_generator = boost::make_shared<HamletGenerator>(base_map);
      break;
    case SETTLEMENT_TYPE_WALLED:
      settlement_generator = boost::make_shared<WalledSettlementGenerator>(base_map);
      break;
    case SETTLEMENT_TYPE_SCATTERED:
      settlement_generator = boost::make_shared<ScatteredSettlementGenerator>(base_map);
      break;
    default:
      break;
  }
