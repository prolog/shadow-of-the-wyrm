...
class ScoreFileEntry : public ISerializable
{
  public:
    ScoreFileEntry();
    ScoreFileEntry(const long long new_score, const std::string& name, const std::string& username, const CreatureSex new_sex, const bool is_current_char, const int lvl, const std::vector<CreatureWin> all_wins, const std::string& race_class_abrv);
    ...
    int get_level() const;
    std::vector<CreatureWin> get_wins() const;
  ...
  protected:
    ...
    int level;
    std::vector<CreatureWin> all_wins;
