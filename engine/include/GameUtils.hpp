...
class GameUtils
{
  public:
    ...
    static void summon_creatures(CreaturePtr caster, MapPtr map, const std::pair<Dice, std::vector<std::string>>& summoned_creature_details, const std::string& message_sid);
    static void move_to_new_map(TilePtr tile, MapPtr old_map, MapPtr new_map, MapExitPtr map_exit = nullptr);
