#include "TileDangerConfirmationKeys.hpp"

using namespace std;

// Confirmation messages for moving on to dangerous tiles
TileDangerConfirmationKeys::TileDangerConfirmationKeys()
{
}

TileDangerConfirmationKeys::~TileDangerConfirmationKeys()
{
}

const string TileDangerConfirmationKeys::TILE_DANGER_MOUNTAINS = "TILE_DANGER_MOUNTAINS";
const string TileDangerConfirmationKeys::TILE_DANGER_AIR = "TILE_DANGER_AIR";
const string TileDangerConfirmationKeys::TILE_DANGER_DEATHLY = "TILE_DANGER_DEATHLY";
