...
void ClassSelectionScreen::initialize()
{
  ...
  // Set the prompt
  PromptPtr class_prompt = std::make_unique<Prompt>(PromptLocation::PROMPT_LOCATION_LOWER_RIGHT);
  class_prompt->set_text_sid(PromptTextKeys::PROMPT_SELECT_AN_OPTION_INFO);
  class_prompt->set_accept_any_input(true);

  user_prompt = std::move(class_prompt);
}
