...
ActionCostValue WeaponInfoAction::weapon_info(CreaturePtr creature, MapPtr map, const WeaponStyle ws) const
{
  ...
  if (creature)
  {
    switch(ws)
    {
      case WeaponStyle::WEAPON_STYLE_MELEE:
        action_cost_value = melee_weapon_info(creature, map);
        break;
      case WeaponStyle::WEAPON_STYLE_RANGED:
        action_cost_value = ranged_weapon_info(creature, map);
        break;
      default:
        break;
    }    
...
ActionCostValue WeaponInfoAction::melee_weapon_info(CreaturePtr creature, MapPtr map) const
{
  IMessageManager& manager = MMF::instance(MessageTransmit::SELF, creature, creature && creature->get_is_player());
  
  if (creature)
  {
    pair<string, string> wielded_offhand_text = get_wielded_and_offhand_text(creature, map);
...
ActionCostValue WeaponInfoAction::ranged_weapon_info(CreaturePtr creature, MapPtr map) const
{
  IMessageManager& manager = MMF::instance(MessageTransmit::SELF, creature, creature && creature->get_is_player());
  
  if (creature)
  {
    string ranged_text = get_ranged_text(creature, map);
...
pair<string, string> WeaponInfoAction::get_wielded_and_offhand_text(CreaturePtr creature, MapPtr map) const
{
  ...
  if (creature != nullptr)
  {
    ...
    wo_text.first = get_melee_weapon_info(creature, map, wielded_weapon, AttackType::ATTACK_TYPE_MELEE_PRIMARY);
    wo_text.second = get_melee_weapon_info(creature, map, off_hand_weapon, AttackType::ATTACK_TYPE_MELEE_SECONDARY);
  }

  return wo_text;
}

string WeaponInfoAction::get_ranged_text(CreaturePtr creature, MapPtr map) const
{
  ...
  if (creature != nullptr)
  {
    ...
    ranged_weapon_text = get_ranged_weapon_info(creature, map, ranged_weapon, ammunition);
  }
...
string WeaponInfoAction::get_melee_weapon_info(CreaturePtr creature, MapPtr map, WeaponPtr weapon, const AttackType attack_type) const
{
  string melee_info;
...
string WeaponInfoAction::get_ranged_weapon_info(CreaturePtr creature, MapPtr map, WeaponPtr ranged_weapon, WeaponPtr ammunition) const
{
